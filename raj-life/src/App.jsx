...
function AppContent() {
  ...
  const { 
    isLocked, unlock, syncing, lastSynced, syncFromSheet, 
    currentMonth, availableMonths, changeMonth, handleCreateMonth, 
    reminders, birthdayReminders, password, passwordLoaded, isInitializing
  } = useApp();
  const [showMonthPicker, setShowMonthPicker] = useState(false);
  const [showReminders, setShowReminders] = useState(false);
  const [showBdayReminder, setShowBdayReminder] = useState(false);
  ...
  useEffect(() => {
    if (!isLocked && birthdayReminders.length > 0) {
      setShowBdayReminder(true);
    }
  }, [isLocked, birthdayReminders]);

  if (isLocked) {
    return <LockScreen onUnlock={unlock} currentPassword={password} />;
  }

  // FIX: Show loading screen instead of black screen while syncing
  if (isInitializing) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-6">
        <div className="text-5xl animate-bounce">✨</div>
        <p className="text-xl font-bold gradient-text">Raj Life</p>
        <p className="text-slate-400 text-sm animate-pulse">Data load ho raha hai...</p>
  ...
  const tabs = [
    ...
    { id: 'birthdays', label: 'Birthday', icon: '🎂' },
  ...
  return (
    <div className="min-h-screen pb-20 md:pb-0 md:pl-20">
          ...
          <div className="glass-card p-6 w-full max-w-sm space-y-4 border-l-4 border-amber-500" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Yeh payments agle 3 din mein due hain</p>
              </div>
            </div>
            ...
            <button onClick={() => setShowReminders(false)} className="glow-button w-full py-3 rounded-xl text-white font-semibold">
              Samajh gaya! 👍
            </button>
          </div>
        </div>
      ...
      {showBdayReminder && birthdayReminders.length > 0 && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4" onClick={() => setShowBdayReminder(false)}>
          <div className="glass-card p-6 w-full max-w-sm space-y-4 border-l-4 border-pink-500" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3">
              <span className="text-3xl">🎂</span>
              <div>
                <h3 className="text-lg font-bold text-pink-400">Birthday Reminder!</h3>
                <p className="text-xs text-slate-400">Inke birthday aane wale hain</p>
              </div>
            </div>
            <div className="space-y-2">
              {birthdayReminders.map(b => {
                ...
                return (
                  <div key={b.id} className="p-3 rounded-xl bg-slate-800/80 flex items-center justify-between">
                    <div>
                    ...
                    <span className="text-sm font-bold text-pink-300">
                      {diff === 0 ? 'Aaj! 🎂' : `${diff} din baaki`}
                    </span>
                  </div>
                );
              })}
            </div>
            <button onClick={() => { setShowBdayReminder(false); setActiveTab('birthdays'); }} className="glow-button w-full py-3 rounded-xl text-white font-semibold">
              Birthday dekhein 🎂
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setShowMonthPicker(false)}>
          <div className="glass-card p-5 w-full max-w-sm space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="border-t border-slate-700 pt-3">
              <p className="text-xs text-slate-400 mb-2">Naya mahina banao:</p>
        ...
        <div className="flex justify-around items-center py-1">
          {tabs.map(tab => (
            <button
              ...
              className={`nav-item flex flex-col items-center py-1 px-1 rounded-xl ${
