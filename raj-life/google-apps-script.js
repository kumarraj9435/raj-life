...
const SPREADSHEET_ID = '1sQdhC6dlVDJYiBkS_lJdgzSHYJ0HGzgf_5VWgabhS-I';
...
function handleRequest(e) {
  try {
    ...
    switch(action) {
      ...
      case 'addBirthday':
        const bdayData = JSON.parse(e.postData.contents);
        result = addBirthday(bdayData.birthday); break;
      case 'deleteBirthday': result = deleteBirthday(params.id); break;
...
// =============================================
// BIRTHDAY FUNCTIONS (Permanent data - no month filter)
// =============================================

// Add 'birthdays' to SHEET_NAMES and HEADERS at the top of the script
// (This is handled separately since birthdays are not month-based)

const BIRTHDAY_SHEET = 'Birthdays';
const BIRTHDAY_HEADERS = ['ID', 'Name', 'Date', 'Relation', 'Notes', 'Created At'];
...
function getBirthdays() {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  let sheet = ss.getSheetByName(BIRTHDAY_SHEET);
  if (!sheet) {
    sheet = ss.insertSheet(BIRTHDAY_SHEET);
    sheet.getRange(1, 1, 1, BIRTHDAY_HEADERS.length).setValues([BIRTHDAY_HEADERS]);
    sheet.getRange(1, 1, 1, BIRTHDAY_HEADERS.length).setFontWeight('bold').setBackground('#ec4899').setFontColor('#ffffff');
    sheet.setFrozenRows(1);
...
function addBirthday(birthday) {
  ...
  const id = Date.now();
  const row = [id, birthday.name, birthday.date, birthday.relation || '', birthday.notes || '', new Date().toISOString()];
  sheet.appendRow(row);
  return { success: true, id: id };
}
...
function deleteBirthday(id) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const sheet = ss.getSheetByName(BIRTHDAY_SHEET);
  if (!sheet) return { error: 'Sheet not found' };
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][0]) === String(id)) {
